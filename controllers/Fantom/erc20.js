...
var web3 = new Web3(new Web3.providers.HttpProvider(config.daemons.fantom.provider));
...
router.post("/transfer", async function (request, response) {
  ...
  let transactionObject = {
    ...
    chainId: config.daemons.fantom.provider.chainId,
  };
...
router.get("/track/:wallet_address/:contract_address", async function (
  ...
  try {
    let tx = await axios.get(
      `${getEtherscanApiUrl(config.daemons.fantom.chainId)}/api?module=account&action=tokentx&contractaddress=${req.params.contract_address}&address=${req.params.wallet_address}&sort=asc&apikey=R3NZBT5BV4WK3VER42TJ3B5UK4WYEDZENH`
    );
