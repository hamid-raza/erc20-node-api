...
var web3 = new Web3(new Web3.providers.HttpProvider(config.daemons.avax.provider));
...
router.post("/transfer", async function (request, response) {
  ...
  console.log(`Avax fee: ${txFee}, balance: ${weiBalance}`);
  if(weiBalance < txFee) {
    return response.status(400).json({
      msg: `Insufficient balance. transaction fee is ${weiBalance} Avax.`,
    });
  }
  ...
  let transactionObject = {
    ...
    chainId: config.daemons.avax.provider.chainId,
  };
        ...
        async function (err, hash) {
          if (!err) {
            ...
            return response.status(200).json({
              msg: "Transaction is in mining state. For more info please watch transaction hash on snowtrace explorer",
              hash: hash,
            });
          } else {
            return response.status(400).json({
...
router.get("/track/:wallet_address/:contract_address", async function (
  ...
  try {
    let tx = await axios.get(
      `${getEtherscanApiUrl(config.daemons.avax.chainId)}/api?module=account&action=tokentx&contractaddress=${req.params.contract_address}&address=${req.params.wallet_address}&sort=asc&apikey=R3NZBT5BV4WK3VER42TJ3B5UK4WYEDZENH`
    );
