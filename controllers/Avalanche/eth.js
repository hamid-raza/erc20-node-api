  ...
  try {
    ...
    ResponseData = {
      wallet: {
        ...
        sent: count,
        received: count,
        link: `https://www.snowtrace.io/address/${walletAddress}`
      },
      message: "",
      timestamp: timestamp,
      status: 200,
...
router.post("/transfer", async function (request, response) {
  ...
  if (fromAddress == "") {
    ...
    return response
      .status(400)
      .json((errors.etherValue = "BNB value is required"));
  } else if (privateKey == "") {
    return response
      .status(400)
  ...
  try {
    ...
    let transactionObject = {
      ...
      chainId: config.daemons.avax.chainId
    };
...
router.get("/track/:wallet_address", async (req, res) => {
  try {
    const result = await axios.get(
      `${getEtherscanApiUrl(config.daemons.avax.chainId)}/api?module=account&action=txlist&address=` +
